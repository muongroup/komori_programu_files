...
void comparehist(){
  ...
  OB1_time=321;
  OB2_time=380;
  std::ifstream file1("BGoutput.txt");//バックグラウンドのデータ
  std::ifstream file2("OBoutput321.txt");//対象物のデータ
  std::ifstream file3("OBoutput380.txt");//対象物のデータ
 ...
 Double_t DIFFERENCE;
  for(int i=0;i<32;i++)
      ...
      for(int j=0;j<32;j++)
	{
	  ...
	  // }
	    // else{
	  hist10->Fill(i-16,j-16,DIFFERENCE);
	  // }
	    }
	}
